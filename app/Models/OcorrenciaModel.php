...
class OcorrenciaModel extends Model
{
    ...
    public function contarMunicipios(): int
    {
        $result = $this->db->table('localizacoes')
            ...
            ->join('ocorrencias', 'ocorrencias.localizacao_id = localizacoes.id', 'inner')
            ->get()->getRow();
    ...
    /**
     * Retorna as Ações de Segurança vinculadas a uma ocorrência.
     */
    public function acoesVinculadas(int $ocorrenciaId): array
    {
        return $this->db->table('ocorrencia_acao oa')
            ->select('oa.momento_vinculo, oa.justificativa, oa.vinculado_em, a.*')
            ->join('acoes_seguranca a', 'a.id = oa.acao_id', 'left')
            ->where('oa.ocorrencia_id', $ocorrenciaId)
            ->get()->getResultArray();
    }
